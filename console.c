...
void cons_runcmd(char *cmdline,struct CONSOLE *cons,int *fat,unsigned int memtotal){
    if(strcmp(cmdline,"mem") == 0 && cons->sht != 0|| strcmp(cmdline,"memory") == 0 && cons->sht != 0){
        cmd_mem(cons,memtotal);
    }else if(strcmp(cmdline,"cls") == 0 && cons->sht != 0|| strcmp(cmdline,"clear") == 0 && cons->sht != 0){
        cmd_cls(cons);
    }else if(strcmp(cmdline,"dir") == 0 && cons->sht != 0|| strcmp(cmdline,"ls") == 0 && cons->sht != 0){
        cmd_dir(cons);
...
int *hrb_api(int edi,int esi,int ebp,int esp,int ebx,int edx,int ecx,int eax){
    ...
    if(edx == 1){
        ...
        ecx = (ecx + 0x0f) & 0xfffffff0;        // 16バイト単位に切り上げ
        memman_free((struct MEMMEN *)(ebx + ds_base),eax,ecx);
    }else if(edx == 11){
        sht = (struct SHEET *)(ebx & 0xfffffffe);
        ...
        for(;;){
            ...
            if(i <= 1){
                ...
                timer_init(cons->timer,&task->fifo,1);      // つぎは1を
                timer_settime(cons->timer,50);
            }
            if(i == 2){
                // カーソルON
