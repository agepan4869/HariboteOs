...
void timer_init(struct TIMER *rimer,struct FIFO32 *fifo,int data);
void timer_settime(struct TIMER *timer,unsigned int timeout);
void inthandler20(int *esp);
int timer_cansel(struct TIMER *timer);
void timer_cancelall(struct FIFO32 *fifo);

// mtask.c
#define MAX_TASKS		100		// 最大タスク数
...
struct TASKCTL{
	...
	int lv_change;				// 次回タスクスイッチのときに、レベルも変えたほうがいいかどうか
